...
__global__ void checkCentroid_CUDA(int N, int D, int iter, int centroid, FLOAT_TYPE *X, FLOAT_TYPE *CTR, FLOAT_TYPE *DIST, int *ASSIGN, FLOAT_TYPE *MAXDIST, int *MAXID)
{
    ...
    FLOAT_TYPE *s_dist    = (FLOAT_TYPE*) array;                // tpb distances --assgn shared memory to another variable?
    int   *s_ID      = (int*)   &s_dist[blockDim.x];  // tpb IDs --local variable?
    FLOAT_TYPE *s_ctr     = (FLOAT_TYPE*) &s_ID[blockDim.x];    // tpb centroid components --local varaible?
    
    unsigned int tid = threadIdx.x;                   // thread ID in block --local variable
    unsigned int t   = blockIdx.x * blockDim.x + tid; // global thread ID --local variable
    ...
    if (t < N)
    {
	    ...
	    //if (offsetD + tid < D) s_ctr[tid] = CTR[offsetD + tid]; 
            ...
            {
                dist += distanceComponentGPU(CTR + d - offsetD, X + (offsetD + d) * N + t);
            }
            offsetD += blockDim.x;
            //__syncthreads();
        }
